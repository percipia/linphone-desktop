...
//----------------------------------------------------------------
	...
	delegate: FocusScope {
		id: itemDelegate
		visible: !mainItem.loading
        height: Utils.getSizeWithScreenRatio(63) + weekHeader.height + topOffset
		width: mainItem.width
		enabled: haveModel
		
		property var itemGui: $modelData
		property var week: $sectionWeek
        ...
        property real topOffset: weekHeader.active ? Utils.getSizeWithScreenRatio(16) : 0
		...
		ColumnLayout {
			anchors.fill: parent
			spacing: itemDelegate.topOffset
			Loader {
				id: weekHeader
				width: parent.width
				active: {
					if (index === 0) return true
					var prev = mainItem.model.data(mainItem.model.index(index - 1, 0), Qt.DisplayRole + 2)
					return prev !== itemDelegate.week
				}
				sourceComponent: Control {
					width: ListView.view ? ListView.view.width : 0
					height: Utils.getSizeWithScreenRatio(19)
					leftPadding: Utils.getSizeWithScreenRatio(55)

					contentItem: Text {
						text: itemDelegate.week
						font: Typography.p1
