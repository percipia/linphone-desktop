...
foreach(_MACOS_ARCH IN LISTS _MACOS_ARCHS)
	...
	add_custom_target(${_TARGET_NAME} ALL
		COMMAND ${CMAKE_COMMAND} -B ${_MACOS_ARCH_BINARY_DIR} -DMONO_ARCH=${_MACOS_ARCH} ${USER_ARGS} ${OPTION_LIST} ${_MACOS_CMAKE_ARGS} -DENABLE_APP_PACKAGING=OFF -DLINPHONEAPP_INSTALL_PREFIX=${_MACOS_ARCH_INSTALL_DIR} -DCMAKE_TOOLCHAIN_FILE=${PROJECT_SOURCE_DIR}/cmake/toolchains/toolchain-mac-${_MACOS_ARCH}.cmake -DLINPHONEAPP_BUILD_TYPE="Normal"
...
set(LINPHONE_ENTITLEMENTS "${PROJECT_SOURCE_DIR}/cmake/install/macos/entitlements.xml.in")
configure_file("${PROJECT_SOURCE_DIR}/cmake/install/macos/linphone_dmg.scpt.in" "${CMAKE_BINARY_DIR}/cmake/install/macos/linphone_dmg.scpt" @ONLY)
configure_file("${PROJECT_SOURCE_DIR}/cmake/install/macos/dmg_sign_prebuild.cmake.in" "${CMAKE_BINARY_DIR}/cmake/install/macos/dmg_sign_prebuild.cmake" @ONLY)
set(LINPHONE_DMG_BACKGROUND "${PROJECT_SOURCE_DIR}/cmake/install/macos/background_dmg.jpg")
set(LINPHONE_DMG_SCPT "${CMAKE_BINARY_DIR}/cmake/install/macos/linphone_dmg.scpt")
set(LINPHONE_DMG_PREBUILD "${CMAKE_BINARY_DIR}/cmake/install/macos/dmg_sign_prebuild.cmake")
if(ENABLE_APP_LICENSE)
	set(LINPHONE_DMG_LICENSE "${PROJECT_SOURCE_DIR}/LICENSE.txt")
else()
	set(LINPHONE_DMG_LICENSE "")
endif()
...
add_custom_target(gen-apps ALL
	...
	"-DENABLE_APP_PACKAGING=${ENABLE_APP_PACKAGING}"
	"-DLINPHONEAPP_VERSION=${LINPHONEAPP_VERSION}"
	"-DLINPHONE_BUILDER_SIGNING_IDENTITY=${LINPHONE_BUILDER_SIGNING_IDENTITY}"
	"-DLINPHONEAPP_SOURCE_DIR=${PROJECT_SOURCE_DIR}"
	"-DLINPHONE_ENTITLEMENTS=${LINPHONE_ENTITLEMENTS}"
	"-DLINPHONE_DMG_BACKGROUND=${LINPHONE_DMG_BACKGROUND}"
	"-DLINPHONE_DMG_SCPT=${LINPHONE_DMG_SCPT}"
	"-DLINPHONE_DMG_PREBUILD=${LINPHONE_DMG_PREBUILD}"
	"-DLINPHONE_DMG_LICENSE=${LINPHONE_DMG_LICENSE}"
